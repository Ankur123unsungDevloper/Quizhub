...
export const QuizCard = ({ card }: { card: CardData }) => {
  ...
  const handleClick = () => {
    incrementViewCount({ topicId: card._id as Id<"topics"> });
  };
    ...
    >
        ...
        <CardContent className="p-0">
          <div className="w-full h-48 md:h-60 overflow-hidden rounded-xl">
            {showImage ? (
              ...
              <div className={`w-full h-full bg-linear-to-br ${fallbackGradients[card.cardType]} flex flex-col items-center justify-center gap-2`}>
      ...
      <div className="flex flex-col py-2 px-1">
        <div className="flex items-center justify-between w-full">
          <span className="text-xs px-2 py-1 rounded-full capitalize text-zinc-500 hover:text-white truncate max-w-[70%]">
            {card.name}
          </span>
        ...
        <div className="flex items-start gap-x-2 mt-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.preventDefault()}>
              <Button variant="ghost" size="icon" className="shrink-0 hover:bg-zinc-700 h-7 w-7">
