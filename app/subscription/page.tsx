...
import { FaLock, FaBrain, FaVideo, FaBookOpen, FaFlask, FaClipboardList } from "react-icons/fa";
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan) => (
              <div key={plan.id}
                className={`
                  relative rounded-2xl border-2 p-6 space-y-5 transition-all duration-200
                  bg-zinc-900 shadow-lg ${plan.glow}
                  ${subscription?.plan === plan.id ? plan.activeBorder : plan.border}
                `}
              >
                {plan.popular && (
